...
docker run -d --network customNet --name messagetest -e MYSQL_ROOT_PASSWORD=$MYSQL_ROOT_PASSWORD -e MYSQL_DB=$MYSQL_DB -e MYSQL_ADDR=$MYSQL_ADDR magdanat/messagingms
